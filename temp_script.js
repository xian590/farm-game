...
function finishTest() {
  ...
  if (testState.type === 'personality' || testState.type === 'personality_deep') {
    ...
    const p = PERSONAS ? (PERSONAS.find(x => x.id === pid) || PERSONAS[0]) : null;
...
function addCustomAffirm() {
  ...
  saveState();
  closeAffirmModal();
  const customAffirm = document.getElementById('custom-affirm-text');
  if (customAffirm) customAffirm.value = '';
...
function generateThreeCardAdvice(cards) {
  ...
  return `这三张牌串起了你的能量流动：从「${cards[0].name}」的过去能量，走到「${cards[1].name}」的当下状态，再走向「${cards[2].name}」的未来可能。整体来看，${cards[2].reversed ? '未来还有一些需要调整的地方' : '未来的走向是积极的'}。重点关注当下的「${cards[1].name}」这张牌给你的提示——它是连接过去和未来的桥梁。${cards[1].advice ? cards[1].advice.slice(0, 50) : ''}... 相信你的直觉，一步一步来，一切都会越来越好的 ✨`;
}
...
function init() {
  ...
  // 深色模式
  if (state.darkMode && document.body) document.body.classList.add('dark');
  ...
  // 全局错误边界 (已在外部定义 window.onerror 和 unhandledrejection)
  // 保留此注释以确保错误处理逻辑清晰
...
function addVipNavEntry() {
  ...
  for (let i = navItems.length - 1; i >= 0; i--) {
    if (navItems[i].parentNode === nav) { lastItem = navItems[i]; break; }
  }
  if(!lastItem) {
    nav.appendChild(vipItem);
    return;
  }
  ...
  if(typeof MOVIE_PRESCRIPTIONS !== 'undefined') {
    MOVIE_PRESCRIPTIONS.forEach((m, i) => {
      if((m.title && m.title.toLowerCase().includes(q)) || (m.theme && m.theme.toLowerCase().includes(q)) || (m.lesson && m.lesson.toLowerCase().includes(q))) {
        results.push({type:'电影',cat:'疗愈',text:m.title, id:'movie_'+i, kind:'movie', data:m.title});
      }
    });
  }
  ...
  if(typeof SATS_SCENES !== 'undefined') {
    SATS_SCENES.forEach((s, i) => {
      if((s.title && s.title.toLowerCase().includes(q)) || (s.desc && s.desc.toLowerCase().includes(q))) {
        results.push({type:'冥想',cat:'SATS',text:s.title, id:'sats_'+i, kind:'sats', data:s.title});
      }
    });
  }
  ...
  setTimeout(() => {
    ...
    banner.innerHTML = `
      <div style="position:fixed;bottom:80px;left:16px;right:16px;z-index:50;background:linear-gradient(135deg,#D4B5C7,#B8A9C9);border-radius:16px;padding:14px 18px;display:flex;align-items:center;gap:12px;box-shadow:0 8px 24px rgba(93,78,109,0.15);color:white;">
        ...
        <button onclick="try{localStorage.setItem('cosmos_pwa_dismissed','true');}catch(e){} document.getElementById('pwa-install-banner').remove();" style="background:rgba(255,255,255,0.2);color:white;border:none;border-radius:10px;padding:8px 10px;font-size:12px;cursor:pointer;">×</button>
      </div>
    `;
...
function render55x5() {
  ...
  container.innerHTML = `
    ...
    <button onclick="if(confirm('确定放弃当前显化挑战？')) { save55x5State({ affirmation: '', currentDay: 1, dailyCount: 0, records: {}, completedChallenges: ${JSON.stringify(state.completedChallenges)} }); render55x5(); showToast('已放弃显化挑战'); }" class="w-full py-2 rounded-lg text-sm" style="background:rgba(232,160,160,0.2); color:#E8A0A0">放弃当前显化挑战</button>
  `;
}

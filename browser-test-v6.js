...
async function getPageId() {
  ...
  for (const p of pages) {
    if (p.type === 'page' && p.url.includes('57885')) return p.id;
  }
  for (const p of pages) {
        ...
        if (opcode === 0x1) {
          ...
          try {
            ...
            if (obj.method === 'Runtime.exceptionThrown') {
              errors.push(obj.params?.exceptionDetails?.exception?.description || JSON.stringify(obj));
            }
...
async function runTests() {
  ...
  try {
    ...
    log('Navigating...');
    await cdp.send('Page.navigate', { url: 'http://127.0.0.1:57885/index.html' });
    await new Promise(r => setTimeout(r, 15000));
    log('Wait complete');

    // Force reload without cache
    log('Reloading without cache...');
    ...
    // Check for JS errors
    results.diagnostics.jsErrors = cdp.errors.slice(0, 10);
    if (cdp.errors.length > 0) {
      log(`JS errors found: ${cdp.errors.length}`);
      cdp.errors.forEach(e => log(`  ERR: ${e.substring(0, 100)}`));
    }
  ...
  if (results.diagnostics.jsErrors && results.diagnostics.jsErrors.length > 0) {
    console.log(`\nJS Errors:`);
    results.diagnostics.jsErrors.forEach(e => console.log(`  ! ${e.substring(0, 150)}`));
  }
}

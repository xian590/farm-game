...
async function getPageId() {
  ...
  for (const p of pages) {
    if (p.type === 'page' && p.url.includes('57799')) return p.id;
  }
  for (const p of pages) {
...
async function runTests() {
  ...
  try {
    ...
    log('Navigating...');
    await cdp.send('Page.navigate', { url: 'http://127.0.0.1:57799/index.html' });
    ...
    // DIAGNOSTIC: Check what globals exist
    ...
    log(`Window keys matching: ${JSON.stringify(windowKeys)}`);

    const consoleErrors = await evalJS(`typeof window.__errors !== 'undefined' ? window.__errors : []`);
    results.diagnostics.consoleErrors = consoleErrors;

...
function connectCDP(pageId) {
  return new Promise((resolve, reject) => {
    ...
    function sendFrame(opcode, data) {
      ...
      let frame;
      if (data.length < 126) {
        frame = Buffer.allocUnsafe(2 + data.length);
        frame[0] = 0x80 | opcode;
        frame[1] = data.length;
        data.copy(frame, 2);
        ...
        frame = Buffer.allocUnsafe(10 + data.length);
        frame[0] = 0x80 | opcode;
        frame[1] = 127;
...
async function runTests() {
  ...
  try {
    ...
    // TEST 2
    try {
      const presets = await evalJS(`(typeof TITLE_PRESETS !== 'undefined') ? Object.keys(TITLE_PRESETS) : null`);
    ...
    try {
      ...
      for (const [id, exp] of Object.entries(expected)) {
        await evalJS(`if(typeof state!=='undefined'){state.titlePreference='${id}';if(typeof saveState==='function')saveState();}`);
        const label = await evalJS(`(typeof getTitle==='function')?getTitle('label'):null`);
    ...
    try {
      const keys = await evalJS(`(typeof VOICE_OPTIONS !== 'undefined') ? Object.keys(VOICE_OPTIONS) : null`);
    ...
    try {
      const dv = await evalJS(`(typeof DEFAULT_STATE !== 'undefined') ? DEFAULT_STATE.voiceType : null`);
    ...
    try {
      const has = await evalJS(`(typeof DEFAULT_STATE !== 'undefined' && Array.isArray(DEFAULT_STATE.microActions))`);
    ...
    try {
      const enStr = await evalJS(`(typeof TRANSLATIONS !== 'undefined' && TRANSLATIONS.en) ? JSON.stringify(TRANSLATIONS.en) : null`);
    ...
    try {
      const has = await evalJS(`typeof broadcastStorageUpdate === 'function'`);
    ...
    // TEST 11: wishDrafts
    try {
      const has = await evalJS(`(typeof DEFAULT_STATE !== 'undefined' && typeof DEFAULT_STATE.wishDrafts === 'object')`);
    ...
    // TEST 12: satsRecords
    try {
      const has = await evalJS(`(typeof DEFAULT_STATE !== 'undefined' && Array.isArray(DEFAULT_STATE.satsRecords))`);

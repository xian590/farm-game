...
function loadState() {
  try {
    ...
    if (saved) {
      ...
      // 确保数组属性始终存在（防止旧数据覆盖为 null）
      const arrayProps = ['wishes','diaries','emotionHistory','badges','unlockedPersonas','testHistory','courseProgress','topBeliefs','moodHistory','cbtRecords','revisionNotes','satsRecords','habits','myTasks','universeTasks','items','rampages','signs','wheels','history','bookmarks','bookNotes','favMovies','movieLinks','cards','planNotes','completedChallenges'];
...
// ==================== v6.4 全局错误处理与加载状态 ====================
const __errEsc = (t) => String(t).replace(/</g, '&lt;').replace(/>/g, '&gt;');
window.onerror = function(msg, url, line, col, error) {
  console.error('Global error:', msg, 'at', url + ':' + line);
  const errHtml = '<div style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:99999;background:rgba(0,0,0,0.85);color:#ff6b6b;padding:20px;font-family:monospace;font-size:13px;overflow:auto;"><h2 style="color:#fff;margin-top:0">🚨 JS Error</h2><pre>' + __errEsc(msg) + '\n@ ' + __errEsc(url) + ':' + line + ':' + col + '\n\nStack: ' + (error && error.stack ? __errEsc(error.stack) : 'N/A') + '</pre><p style="color:#aaa">请截图此错误信息发给开发者</p></div>';
...
window.addEventListener('unhandledrejection', function(event) {
  ...
  const errHtml = '<div style="position:fixed;top:0;left:0;right:0;bottom:0;z-index:99999;background:rgba(0,0,0,0.85);color:#ff6b6b;padding:20px;font-family:monospace;font-size:13px;overflow:auto;"><h2 style="color:#fff;margin-top:0">🚨 Unhandled Promise Rejection</h2><pre>' + __errEsc(event.reason) + '</pre><p style="color:#aaa">请截图此错误信息发给开发者</p></div>';
  const d = document.createElement('div');
  d.innerHTML = errHtml;
  document.body.appendChild(d);
});
...
function openWishDetail(i) {
  ...
  setText('wd-title', w.done ? '✨ 已显化的愿望' : '🌠 进行中的愿望');
...
function __diagLog(step, ok, err) {
  try {
    console.log('[Init]', ok ? '✅' : '❌', step, err || '');
    let el = document.getElementById('__init_diag');
    if (!el) {
      el = document.createElement('div');
      el.id = '__init_diag';
      el.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99998;background:#1a1625;color:#e8e0f0;font-family:monospace;font-size:13px;padding:10px;max-height:150px;overflow:auto;white-space:pre-wrap;line-height:1.6;border-bottom:2px solid #ff6b6b;';
      document.body.appendChild(el);
    }
    const span = document.createElement('span');
    span.style.display = 'block';
    span.textContent = (ok ? '✅ ' : '❌ ') + step + (err ? ' | ' + err : '');
    el.appendChild(span);
  } catch(e) { console.error('__diagLog failed:', e); }
}

function init() {
  const steps = [
    ['createFireflies', () => createFireflies()],
    ['updateTopbar', () => updateTopbar()],
    ['updateGreeting', () => updateGreeting()],
    ['dailyAffirm text', () => { const dailyEl = document.getElementById('daily-affirm-text'); if (dailyEl) dailyEl.textContent = state.dailyAffirm; }],
    ['updateMoodDisplay', () => updateMoodDisplay()],
    ['restore mood detail', () => {
      if (state.todayMood && state.todayMood.date === getTodayStr()) {
    ...
    ['updateTimeAndWeather', () => updateTimeAndWeather()],
    ...
    ['dark mode', () => { if (state.darkMode) document.body.classList.add('dark'); }],
    ['animation toggle', () => { if (!state.animation) document.documentElement.style.setProperty('--animation-duration', '0s'); }],
    ...
    ['renderSmartRecommendations', () => renderSmartRecommendations()],
    ...
    ['interval timers', () => {
      setInterval(() => { updateTimeAndWeather(); checkReminders(); }, 60000);
    }],
    ['welcome/home', () => {
    ...
    ['updateFortuneCard', () => updateFortuneCard()],
  ...
  for (const [name, fn] of steps) {
    try {
      ...
      __diagLog(name, false, err.message);
...
// init() moved to end of Block 3 to ensure all functions are defined
